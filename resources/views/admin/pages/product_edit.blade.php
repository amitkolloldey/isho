            ...
            <div class="tab-content" id="myTabContent">
                <div class="tab-pane fade show active" id="general" role="tabpanel" aria-labelledby="general-tab">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control" id="price" placeholder="Product Price" name="price" value="{{$product->price}}">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <input type="hidden" id="main_image_name" name="main_image_name" value="{{$product->main_image}}">
                                <img src="{{config('app.url')}}/{{$product->main_image}}" alt="" id="main_image_src" width="50px">
                            </div>
                            <div class="col-md-6">
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" id="description" cols="30" rows="10" name="description">{{$product->description}}</textarea>
                    </div>
                </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="{{$value['value']}}-sku" placeholder="SKU"
                                           name="attribute_values[{{$value['id']}}][sku]" @if(isset($product_values[$value['id']])) value="{{$product_values[$value['id']]['sku']}}" @endif>
                                </div>
                                <div class="form-group">
                                    <label for="{{$value['value']}}-price">Price</label>
                                    <input type="number" class="form-control" id="{{$value['value']}}-price"
                                           placeholder="Price" name="attribute_values[{{$value['id']}}][price]" @if(isset($product_values[$value['id']])) value="{{$product_values[$value['id']]['price']}}" @endif>
                                </div>
                            </div>
                            <div class="form-group">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        ...
                                        <input type="hidden" id="attribute_{{$value['id']}}_name" name="attribute_values[{{$value['id']}}][image_src]" @if(isset($product_values[$value['id']])) value="{{$product_values[$value['id']]['image']}}" @endif >
                                        <img @if(isset($product_values[$value['id']])) src="{{config('app.url')}}/{{$product_values[$value['id']]['image']}}" @endif  id="attribute_{{$value['id']}}_src" width="50px">
                                    </div>
                                    <div class="col-md-6">
                                        <button class="btn btn-danger"
                ...
                <button onclick="event.preventDefault(); update()" class="btn-success btn">Save</button>
            </div>
        </form>
    </div>
@stop
...
@section('scripts')
    <script>
        ...
        function update() {
            $('#loader').show();
            var formData = $("#product_edit").serialize()
            console.log(formData)
            axios.put('/api/product/update/'+{{$product->id}}, formData
                , {
                    headers: {
                        'Accept': 'application/json',
                        'Authorization': 'Bearer ' + '{{session()->get('access_token')}}'
